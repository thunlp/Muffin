...
from muffin.constants import WORKER_HEART_BEAT_INTERVAL
...
def load_model(model_path, num_gpus):
    ...
    if 'llava' in model_path.lower():
        model = Beit3LlavaLlamaForCausalLM.from_pretrained(model_path, torch_dtype=torch.float16, low_cpu_mem_usage=True, **kwargs)
    else:
    ...
    if True:
        ...
        image_processor = build_transform(is_train=False, input_size=model.model.vision_tower.args.img_size)
        ...
        if vision_tower.beit3.text_embed.weight.device.type == 'meta':
            vision_tower = timm.create_model(model.model.config.mm_vision_tower)
            state_dict = torch.load('/mnt/data/user/tc_agi/multi_modal/checkpoints/beit-v3/beit3_large_patch16_224.pth')['model']
...
class ModelWorker:
    ...
    def generate_stream(self, params):
        ...
        # print(f'#img is {prompt.count(DEFAULT_IMAGE_TOKEN)}, prompt is {prompt}, params={params.keys()}')
        ...
        if images is not None and self.is_multimodal:
            ...
            if len(images) > 0:
                ...
                if self.keep_aspect_ratio:
                    ...
                    replace_token = DEFAULT_IMAGE_PATCH_TOKEN * 64    # HACK: 64 is the max image token length hacked
                    if getattr(self.model.config, 'mm_use_im_start_end', False):
                        replace_token = DEFAULT_IM_START_TOKEN + replace_token + DEFAULT_IM_END_TOKEN
        ...
        print(f'Origin prompt is {repr(params["prompt"])}')
        print(f'Temperature is {temperature}. Prompt is {repr(prompt)}')
