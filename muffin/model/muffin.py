...
class Beit3LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        super(Beit3LlavaLlamaModel, self).__init__(config)
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            # state_dict = load_model_and_may_interpolate(state_dict, self.vision_tower)
        ...
        if hasattr(config, "num_query"):
            assert config.num_query > 2
    ...
    def forward(
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            mask_query2img = False
            mask_query2img = int(os.environ.get('MASK_QUERY2IMG', '0'))
            if mask_query2img:
                assert False
                # image cannot (watch=>) query
                image_len = vision_tower.beit3.vision_embed.num_position_embeddings()
                max_len = self.config.num_query + image_len
                ...
                # full attention for query (watch=>) query
                uni_mask[t_start:t_end, t_start:t_end] = 1

                # full attention for query (watch=>) image
                uni_mask[t_start:t_end, i_start:i_end] = 1

                # full attention for image (watch=>) image
            ...
            dummy_image_features = torch.zeros(self.config.num_query, vision_tower.hidden_size, device=inputs_embeds.device, dtype=inputs_embeds.dtype)
